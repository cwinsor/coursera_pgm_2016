...
%
%for ass=1:prod(A.card)
%	  printf("ass %d indxB(ass) %d\n",ass,indxB(ass));
%end
%
%for ass = 1:prod(A.card)
%	    ass_set = assignments(ass,:);
%	    printf("ass %d GetValueOfAssignment(A, ass_set) %d\n", ass, GetValueOfAssignment(A, ass_set) );
%end

for ass = 1:prod(A.card)
	    ass_set = assignments(ass,:);
            B.val(indxB(ass)) = B.val(indxB(ass)) + GetValueOfAssignment(A, ass_set);
end
