...
%for ass = 1:prod(C.card)
%	    printf("ass %d assignments(ass) %d\n",ass, AssignmentToIndex(assignments(ass,:),C.card));
%end
%
%for ass = 1:prod(C.card)
%	    printf("ass %d assignments(ass) %d\n", ass, AssignmentToIndex(assignments(ass,:),C.card));
...
%
%for ass = 1:prod(C.card)
%	    printf("ass %d indxA() %d\n", ass, indxA(AssignmentToIndex(assignments(ass,:),C.card)));
%end
%
%for ass = 1:prod(C.card)
%   printf("ass %d indxA() %d %d\n", ass,
%	  indxA(AssignmentToIndex(assignments(ass,:),C.card)),
%	  indxB(AssignmentToIndex(assignments(ass,:),C.card)));
%end
%
%
%for ass = 1:prod(C.card)
%   printf("ass %d %d\n", ass,
%	  A.val(indxA(AssignmentToIndex(assignments(ass,:),C.card))) *
%	  B.val(indxB(AssignmentToIndex(assignments(ass,:),C.card))));
%end

%%%% todo - vectorize it
for ass = 1:prod(C.card)
	    C.val(ass) =   A.val(indxA(AssignmentToIndex(assignments(ass,:),C.card))) *  B.val(indxB(AssignmentToIndex(assignments(ass,:),C.card)));
end
